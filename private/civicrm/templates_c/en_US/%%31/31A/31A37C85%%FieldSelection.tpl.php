...
<?php $this->_tag_stack[] = array('crmScope', array('extensionKey' => "")); $_block_repeat=true;smarty_block_crmScope($this->_tag_stack[count($this->_tag_stack)-1][1], null, $this, $_block_repeat);while ($_block_repeat) { ob_start(); ?>
  <div id="report-tab-col-groups" class="civireport-criteria">
    <?php $_from = $this->_tpl_vars['colGroups']; if (($_from instanceof StdClass) || (!is_array($_from) && !is_object($_from))) { settype($_from, 'array'); }if (count($_from)):
    foreach ($_from as $this->_tpl_vars['dnc'] => $this->_tpl_vars['grpFields']):
?>
      <?php $this->assign('count', '0'); ?>
            <?php if (! empty ( $this->_tpl_vars['grpFields']['use_accordian_for_field_selection'] )): ?>
...
">
          <?php $_from = $this->_tpl_vars['grpFields']['fields']; if (($_from instanceof StdClass) || (!is_array($_from) && !is_object($_from))) { settype($_from, 'array'); }if (count($_from)):
