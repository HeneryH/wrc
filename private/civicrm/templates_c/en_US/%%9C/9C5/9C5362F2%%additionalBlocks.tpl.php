...
CRM.$(function($) {
    ...
    <?php if ($this->_tpl_vars['generateAjaxRequest']): ?>
        <?php $_from = $this->_tpl_vars['ajaxRequestBlocks']; if (($_from instanceof StdClass) || (!is_array($_from) && !is_object($_from))) { settype($_from, 'array'); }if (count($_from)):
    foreach ($_from as $this->_tpl_vars['blockName'] => $this->_tpl_vars['instances']):
?>
            <?php $_from = $this->_tpl_vars['instances']; if (($_from instanceof StdClass) || (!is_array($_from) && !is_object($_from))) { settype($_from, 'array'); }if (count($_from)):
...
' );
    ...
    <?php if (! empty ( $this->_tpl_vars['loadShowHideAddressFields'] )): ?>
        <?php $_from = $this->_tpl_vars['showHideAddressFields']; if (($_from instanceof StdClass) || (!is_array($_from) && !is_object($_from))) { settype($_from, 'array'); }if (count($_from)):
...
function buildAdditionalBlocks( blockName, className ) {
    ...
    var dataUrl = '; ?>
"<?php echo smarty_function_crmURL(array('h' => 0,'q' => 'snippet=4'), $this);?>
