...
function cividiscount_civicrm_buildForm($fname, &$form) {
  ...
  if (in_array($fname, [
    ...
    if ($form->getVar('_single') == 1 || in_array($form->getVar('_context'), ['membership', 'standalone'])) {
      ...
      if (in_array($fname, ['CRM_Member_Form_Membership', 'CRM_Member_BAO_Membership', 'CRM_Member_Form_MembershipRenewal'])) {
  ...
  if ((!$discountInfo || !$discountInfo['autodiscount']) && trim($code) != '') {
    ...
    if ($discount['count_max'] > 0) {
      ...
      if (array_key_exists('additional_participants', $sv)) {
        $apcount += intval($sv['additional_participants']);
      }
      ...
      if (!empty($autodiscount) || !empty($code)) {
        foreach ($amounts as $fee_id => &$fee) {
          ...
          foreach ($fee['options'] as $option_id => &$option) {
            if (!empty($applyToAllLineItems) || !empty($priceFields[$option['id']])) {
          ...
          if (($discountAppliedForPriceFieldValues ?? []) == array_keys($fee['options'])) {
            $applyToAllLineItems = TRUE;
          }
        }
      }
...
function cividiscount_civicrm_postProcess($class, &$form) {
  ...
  $discountParams = [
    'item_id' => $discount['id'],
    'description' => ($params['amount_level'] ?? '') . ' ' . ($params['description'] ?? ''),
...
function _cividiscount_consume_discount_code_for_online_contribution($params, $discountParams, $discountedMemberships) {
  ...
  if (empty($discount_membership_matches) || empty($params['createdMembershipIDs'])) {
    return;
  }

  foreach ($params['createdMembershipIDs'] as $membershipId) {
...
function cividiscount_civicrm_permission(&$permissions) {
  ...
  $permissions['administer CiviDiscount'] = [
    $prefix . E::ts('administer CiviDiscount'),
    E::ts('Administer the CiviDiscount extension'),
  ];
}
