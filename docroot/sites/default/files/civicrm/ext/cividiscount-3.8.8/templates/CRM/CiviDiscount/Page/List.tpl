      ...
      <table id="options" class="display">
        ...
        {foreach from=$rows item=row}
          <tr id="discount_code-{$row.id}" class="crm-entity {$row.class}{if NOT $row.is_active} disabled{/if}">
            ...
            <td data-sort="{$row.active_on|truncate:10:''}">{if $row.active_on neq '0000-00-00 00:00:00'}{$row.active_on|truncate:10:''|crmDate}{/if}</td>
            <td data-sort="{$row.expire_on|truncate:10:''}">{if $row.expire_on neq '0000-00-00 00:00:00'}{$row.expire_on|truncate:10:''|crmDate}{/if}</td>
...
<div class="action-link">
  <a href='{crmURL p='civicrm/cividiscount/discount/add' q="reset=1"}' id="newDiscountCode"
     class="button">{icon icon="fa-plus-circle"}{/icon} {ts}New Discount Code{/ts}</a>
</div>
